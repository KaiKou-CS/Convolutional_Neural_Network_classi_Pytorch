...
def train():
    os.makedirs(model_cp, exist_ok=True)
    ...
    model = vgg19_Net(in_img_rgb=3, in_img_size=64, out_class=2, in_fc_size=18432)  # 实例化一个网络
    ...
    for epoch in range(epoch_count):
        ...
        for img, label in dataloader:  # 循环读取封装后的数据集，其实就是调用了数据集中的__getitem__()方法，只是返回数据格式进行了一次封装
            ...
            print('预测  out  : ', out.argmax(dim=1))
            print('实际label  : ', label.squeeze())
            p = out.argmax(dim=1).eq(label.squeeze())
            print('预测正确个数： ', p.sum())

            # print('label.squeeze():  ',label.squeeze().shape,label)
            ...
            print(epoch, 'Frame {0}, train_loss {1}'.format(cnt * batch_size, loss / batch_size), '\n')  # 打印一个batch size的训练结果

        torch.save(model.state_dict(), './{0}/vgg19_model_{1}.pth'.format(model_cp, epoch))  # 训练所有数据后，保存网络的参数
